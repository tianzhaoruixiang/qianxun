...
@Service
public class QianXunServiceActivityLog {

    private static final Logger log = LoggerFactory.getLogger(QianXunServiceActivityLog.class);
    private static final int MAX_LOGS = 200;

    private final ChatActivityLogRepository logRepo;
    private final MessageFeedbackRepository feedbackRepo;

    public QianXunServiceActivityLog(
    ...
    public List<ActivityLogResponse> listBySession(String sessionId) {
        return logRepo.listBySessionId(sessionId, MAX_LOGS)
                .stream()
                .map(QianXunServiceActivityLog::toResponse)
                .toList();
    }

    public Optional<ActivityLogResponse> findByAssistantMessage(String assistantMessageId) {
        return logRepo.findByAssistantMessageId(assistantMessageId).map(QianXunServiceActivityLog::toResponse);
    }
    ...
    public Optional<FeedbackResponse> getFeedback(String messageId) {
        return feedbackRepo.findByMessageId(messageId).map(QianXunServiceActivityLog::toFeedbackResponse);
    }
