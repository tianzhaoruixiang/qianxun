...
@Service
public class QianXunChatSessionService {
    ...
    public QianXunChatSessionService(
    ...
    public List<ChatSessionResponse> list() {
        ...
        return sessionRepository.listByUserIdOrderByUpdatedDesc(userId, MAX_LIST)
                .stream().map(QianXunChatSessionService::toResponse).toList();
    }

    public ChatSessionResponse get(String id) {
        String userId = UserContext.getCurrentUserId();
        return sessionRepository.findByIdAndUserId(id, userId)
                .map(QianXunChatSessionService::toResponse)
    ...
    public ChatSessionResponse update(String id, UpdateSessionRequest request) {
        ...
        if (request == null || request.title() == null || request.title().isBlank()) {
            return sessionRepository.findByIdAndUserId(id, userId)
                    .map(QianXunChatSessionService::toResponse).orElseThrow();
        }
        Instant now = Instant.now();
    ...
    public List<ChatMessageResponse> listMessages(String sessionId) {
        ...
        return messageRepository.listBySessionOrderByCreatedAsc(sessionId, MAX_MESSAGES)
                .stream().map(QianXunChatSessionService::toMessageResponse).toList();
    }

    // ── 内部接口（不经过 UserContext，供 QianXunChatStreamService 等内部逻辑使用）────────
    ...
    public ChatMessage appendUserMessage(String sessionId, String content) {
        ...
        Instant now = Instant.now();
        String id   = newId();
        ChatMessage message = new ChatMessage(id, sessionId, "user", content, ChatMessage.MODE_QUICK, null, now);
    ...
    public ChatMessage appendAssistantMessage(
            String sessionId, String content, String thinkingMode, String thinkContent
    ) {
        Instant now = Instant.now();
        String id   = newId();
        ChatMessage message = new ChatMessage(
                id, sessionId, "assistant", content, thinkingMode, thinkContent, now
        );
        messageRepository.insert(message);
        sessionRepository.touchUpdatedAt(sessionId, now);
        return message;
    ...
    private static ChatMessageResponse toMessageResponse(ChatMessage m) {
        return new ChatMessageResponse(
                ...
                m.thinkingMode(), m.thinkContent(), m.createdAt()
        );
    }
