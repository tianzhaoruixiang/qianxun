...
public class ChatMessageRepository {
    ...
    public void insert(ChatMessage message) {
        jdbcTemplate.update(
                "INSERT INTO " + table
                + " (`id`,`session_id`,`role`,`content`,`thinking_mode`,`think_content`,`created_at`)"
                + " VALUES (?,?,?,?,?,?,?)",
                message.id(), message.sessionId(), message.role(), message.content(),
                message.thinkingMode(), message.thinkContent(),
    ...
    public List<ChatMessage> listBySessionOrderByCreatedAsc(String sessionId, int limit) {
        return jdbcTemplate.query(
                "SELECT `id`,`session_id`,`role`,`content`,`thinking_mode`,`think_content`,`created_at`"
                + " FROM " + table
