...
├── nginx.conf              # 前端 nginx 配置（SSE 友好的反向代理 /api → 后端）
├── doris/
...
| qianxun-frontend| qianxun-frontend      | 5173       | Vite 静态站 + nginx 反代 `/api` |
...
- 后端 `/api/sessions`、`/api/intent-scenarios`
- 前端首页 200、`/api` 反向代理可用
...
### 前端 SSE
- nginx 已设 `proxy_buffering off / chunked_transfer_encoding off / X-Accel-Buffering: no`；浏览器对 `/api/*` 的请求由 nginx 透传至后端 8080，不存在跨域。
...
| 后端 403/无 SSE | 确认 `.env` 中 `API_SERVER_KEY` 与后端 `HERMES_API_KEY` 一致；前端走 `/api` 默认无需配置 |
